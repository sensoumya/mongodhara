...
def validate_database_name_for_create(db_name: str) -> None:
    ...
    if len(db_name.encode("utf-8")) > 64:
        raise HTTPException(
            status_code=400,
            detail="Database name cannot exceed 64 bytes when UTF-8 encoded",
        )
    ...
    for char in invalid_chars:
        if char in db_name:
            raise HTTPException(
                status_code=400,
                detail=f"Database name cannot contain '{char}' character",
            )
    ...
    if db_name.lower() in reserved_names:
        raise HTTPException(
            status_code=400, detail=f"'{db_name}' is a reserved database name"
        )
            ...
            if version_match:
                ...
                try:
                    ...
                    if decoded_path and ("/" in decoded_path or "?" in decoded_path):
                        ...
                        logger.debug(
                            f"RFC 4648 Section 5 decoded {version_prefix} path: {full_path} -> {request.scope['path']}"
                        )

                except (binascii.Error, UnicodeDecodeError, ValueError) as e:
                    # If decoding fails, continue with original path
                    logger.debug(
                        f"Base64 decoding failed for {version_prefix} path (expected for normal routes): {e}"
                    )
                    pass
...
def list_databases(
    ...
    try:
        return mongo.list_databases(
            search, sort, page, page_size, sort_field, sort_order
        )
    except Exception as e:
...
@v1_router.post(
    "/db/{db}", summary="Create a new database", tags=["Database Management"]
)
def create_database(
    db: str,
    collection_name: str = Query(
        ..., description="Name of the initial collection to create in the database"
    ),
):
    ...
    try:
        ...
        return {
            "message": f"Database '{db}' created successfully with collection '{collection_name.strip()}'"
        }
        ...
        logger.error(
            f"Collection validation error for '{collection_name}' in DB '{db}': {e}"
        )
        raise HTTPException(
            status_code=400,
            detail=f"Invalid collection name '{collection_name}': {str(e)}",
        )
        ...
        logger.error(
            f"Failed to create database '{db}' with collection '{collection_name}': {e}",
            exc_info=True,
        )
...
def create_collection(db: str, col: str):
    ...
    try:
        ...
        if "already exists" in error_msg.lower():
            ...
            raise HTTPException(
                status_code=409,
                detail=f"Collection '{col}' already exists in database '{db}'",
            )
        else:
            logger.error(f"Collection validation error for '{col}' in DB '{db}': {e}")
            raise HTTPException(
                status_code=400, detail=f"Invalid collection name '{col}': {str(e)}"
            )
    except OperationFailure as e:
        ...
        if (
            ...
            logger.error(
                f"MongoDB operation failed for collection creation '{col}' in DB '{db}': {e}"
            )
            raise HTTPException(
                status_code=400,
        ...
        logger.error(
            f"Failed to create collection '{col}' in DB '{db}': {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail="Failed to create collection")


@v1_router.delete(
    "/db/{db}/col/{col}", summary="Delete a collection", tags=["Collection Management"]
)
...
def import_documents(
    ...
    try:
        ...
        if isinstance(json_data, list):
            ...
            raise HTTPException(
                status_code=400,
                detail="JSON must be either an array of documents or contain a 'documents' key",
            )
...
@v1_router.post(
    "/db/{db}/gridfs/{bucket_name}/upload",
    summary="Upload a file to a specific GridFS bucket",
    tags=["GridFS File Storage"],
)
...
@v1_router.get(
    ...
    summary="Download file by ID from a specific bucket",
    tags=["GridFS File Storage"],
)
...
@v1_router.get(
    "/db/{db}/gridfs/{bucket_name}/filename/{filename}/download",
    summary="Download latest file by filename from a specific bucket",
    tags=["GridFS File Storage"],
)
def download_file_by_name_from_bucket(db: str, bucket_name: str, filename: str):
    ...
    try:
        ...
        logger.error(
            f"Failed to download file '{filename}' from bucket '{bucket_name}' in DB '{db}': {e}",
            exc_info=True,
        )
        raise HTTPException(status_code=500, detail="Failed to download file")
...
@v1_router.delete(
    "/db/{db}/gridfs/{bucket_name}/filename/{filename}",
    summary="Delete all files by filename from a specific bucket",
    tags=["GridFS File Storage"],
)
def delete_files_by_name_from_bucket(db: str, bucket_name: str, filename: str):
    ...
    try:
        deleted_count = mongo.delete_files_by_name_from_bucket(
            db, bucket_name, filename
        )
        if deleted_count == 0:
            raise HTTPException(
                status_code=404, detail="No files found with that filename"
            )
        return {
            "message": f"Deleted {deleted_count} files named '{filename}' from bucket '{bucket_name}'"
        }
    except Exception as e:
        logger.error(
            f"Failed to delete files named '{filename}' from bucket '{bucket_name}' in DB '{db}': {e}",
            exc_info=True,
        )
