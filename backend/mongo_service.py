...
class MongoService:
    ...
    def list_collections(self, db_name, search=None, sort="asc", page=1, page_size=10, sort_field=None, sort_order=1):
        try:
            ...
            collections = [c for c in collections if not c.startswith('system.') and not c.endswith('.files') and not c.endswith('.chunks')]
        ...
        for collection in collections:
            if collection.endswith('.files'):
                ...
                if chunks_collection in collections:
                    ...
                    size_result = list(self.client[db_name][collection].aggregate(pipeline))
    ...
    def upload_file_to_bucket(self, db_name, bucket_name, file_content, filename, content_type=None, metadata=None):
        """Upload a file to a specific GridFS bucket"""
        logger.info(f"Uploading file '{filename}' to GridFS bucket '{bucket_name}' in DB '{db_name}'")
    ...
    def download_file_from_bucket(self, db_name, bucket_name, file_id):
        ...
        logger.debug(f"Downloading file with ID '{file_id}' from GridFS bucket '{bucket_name}' in DB '{db_name}'")
        fs = gridfs.GridFS(self.client[db_name], collection=bucket_name)
    ...
    def download_file_by_name_from_bucket(self, db_name, bucket_name, filename):
        ...
        logger.debug(f"Downloading file '{filename}' from GridFS bucket '{bucket_name}' in DB '{db_name}'")
        fs = gridfs.GridFS(self.client[db_name], collection=bucket_name)
    ...
    def list_files_in_bucket(self, db_name, bucket_name, search=None, page=1, page_size=10):
        """List files in a specific GridFS bucket with pagination and search"""
        logger.debug(f"Listing GridFS files in bucket '{bucket_name}' in DB '{db_name}' with pagination")
        ...
        cursor = self.client[db_name][files_collection].find(query).skip(skip).limit(page_size).sort('uploadDate', -1)
    ...
    def get_file_metadata_from_bucket(self, db_name, bucket_name, file_id):
        ...
        logger.debug(f"Getting metadata for file ID '{file_id}' from GridFS bucket '{bucket_name}' in DB '{db_name}'")
        files_collection = f"{bucket_name}.files"
    ...
    def delete_file_from_bucket(self, db_name, bucket_name, file_id):
        ...
        logger.warning(f"Deleting file with ID '{file_id}' from GridFS bucket '{bucket_name}' in DB '{db_name}'")
        fs = gridfs.GridFS(self.client[db_name], collection=bucket_name)
    ...
    def delete_files_by_name_from_bucket(self, db_name, bucket_name, filename):
        ...
        logger.warning(f"Deleting all files named '{filename}' from GridFS bucket '{bucket_name}' in DB '{db_name}'")
    ...
    def delete_bucket(self, db_name, bucket_name):
        ...
        logger.warning(f"Deleting entire GridFS bucket '{bucket_name}' from DB '{db_name}'")
